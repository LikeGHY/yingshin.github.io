...
glog是google的的一个C++轻量日志库，支持常用的日志功能：例如日志级别，自定义级别，条件输出,DEBUG日志等  
本文主要介绍glog的一些基本用法。  
...
默认的输出位置在tmp下，具体为：  
/tmp/<program name>.<hostname>.<username>.log.<severity level>.<data>.<time>.<pid>，比如:introduction.ymac.yingshin.log.ERROR.20160102-231949.11182


### 日志级别
日志级别有4种，INFO WARNING ERROR FATAL，在`glog/log_severity.h`中定义
...
注意还有一种日志级别是DFATAL，这种级别在debug模式下为ERROR，当定义NDEBUG时则是FATAL，例如：

```
LOG(DFATAL) << "Hello, glog fatal.";
```
主要通过在NDEBUG下定义为ERROR，否则为FATAL来实现。  

```
// glog/log_severity.h里的定义
...
glog有很多配置项，这些配置项都是通过gflags设置的，关于gflags的使用可以参考[这篇文章](http://yingshin.github.io/c/cpp/2015/11/15/gflags-introduction/)。  
...
同时，更精彩的是自定义日志可以定义到文件级别，使用--vmodule=mapreduce=2,file=1,gfs*=3  
...
`CHEKCK(expression) << "check failed description"`，检测expression如果不为真，则打印后面的description和栈上的信息，然后退出程序，出错后的处理过程和FATAL比较像。  
除了CHECK，还有CHECK\_EQ,CHECK\_NE,CHECK\_LE,CHECK\_LT,CHECK\_GE,CHECK\_GT一系列宏用于判断。  

空指针的判断需要转化下，例如`CHECK_EQ(p, static_cast<int*>(NULL)`，直接`CHECK_EQ(p, NULL)`会报错。

字符串的比较需要使用CHECK\_STREQ,CHECK\_STRNE,CHECK\_STRCASEEQ,CHECK\_STRCASENE，注意这种情况下可以传入NULL，例如CHECK\_STREQ(NULL, NULL)可以正常运行。  

###参考文档：  
1. [glog官方文档](http://google-glog.googlecode.com/svn/trunk/doc/glog.html)  
