...
除了CHECK，还有CHECK_EQ,CHECK_NE,CHECK_LE,CHECK_LT,CHECK_GE,CHECK_GT一系列宏用于判断。  

空指针的判断需要转化下，例如`CHECK_EQ(p, static_cast<int*>(NULL)`，直接`CHECK_EQ(p, NULL)`会报错。

字符串的比较需要使用CHECK_STREQ,CHECK_STRNE,CHECK_STRCASEEQ,CHECK_STRCASENE，注意这种情况下可以传入NULL，例如CHECK_STREQ(NULL, NULL)可以正常运行。  
