...
条件输出是指，当某些变量满足某些条件时才输出日志。我在实际使用时也经常有这种需求，有的请求失败希望能够输出，但是全部输出磁盘IO可能又有问题，使用条件输出可以起到一个抽样的作用。  
...
注意`google::COUNTER`并不总是生效的，例如这样输出google::COUNTER没有意义，数据全为0，使用时需要确认下。
