---
layout: post
title: MIT 6.824 Distributed Systems Lec-1 学习笔记
date: 2017-07-18 19:50:32
excerpt: "MIT 6.824 Distributed Systems Lec-1 学习笔记"
categories: [Distributed Systems]
tags: [go mapreduce 6.824]
---
...
网上全部课程由23节lecture，4个lab组成，校内课程还包括exam和project.

课程主要目的主要是跟进各节lecture，看论文，完成实验，在动手过程中了解分布式系统。
...
在[日历](https://pdos.csail.mit.edu/6.824/schedule.html)里的第一节课就是mapreduce。
...
![Figure 1: Execution overview](http://imgsrc.baidu.com/baike/pic/item/5882b2b7d0a20cf49b9b0c7b76094b36acaf990f.jpg)

从网上找了一个论文里的图:Figure 1: Execution overview，其中worker封装了文件的读写，以及用户`map/reduce`函数的调用，搞清楚上下游关系后，lab里主要的任务也是围绕worker的实现完成。

## 3. lab

lab1分为5个part，Collaboration Policy要求不要公布代码，因此这里逐个part说下思路。建议也是自己动手完成，作者还是很用心的，毕竟看了论文再实现一遍的机会并不多。
...
lab1的要求是在第四节课之后完成，不过目前mr理论已经足够成熟了，因此做起来不会太费劲。这个课程设计也非常赞，从`schedule`分发任务，到`doMap/doReduce`调用用户函数，到`mapF/reduceF`实现用户函数，整个做下来对mapreduce流程可以有一个更详细的了解，对go语言也有一个入门。
