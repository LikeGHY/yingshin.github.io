...
Raft是解决多个实例上的日志一致性算法，在Raft论文里分为了`leader election`, `log replication`两部分，同时还介绍了snapshot以及集群实例数变更的方案。
...
每个测试运行3min左右，跑了1000次，有1次失败，其余pass。

## 3. lab2实战

尽管Raft确实相比Paxos在理解性上强了很多，但是完成lab2的各项测试在我看来依然不简单。在[student guide](https://thesquareplanet.com/blog/students-guide-to-raft/)部分给出的上手建议是这样的：
...
整体上需要关注的是3个状态：leader candidate follower，以及两个RPC的实现:AppendEntries RequestVote。
...
实现上根据测试用例逐步推进，例如2A的实现里先不用考虑论文里提到的`Election restrction`。同时测试会越来越严格，对一些`corner case`也都能覆盖到（测试使用了很多随机sleep/disconnect/latency等行为，因此需要多次运行验证）。
...
2. **Follower**:leader的从属角色，不主动发起RPC，响应其他实例的RPC：leader选举，日志复制等。  
...
**收到更高term的RPC，无论是RequestVote还是AppendEntries**

当收到的RequestVote或者AppendEntries里包含的term，<=自身的currentTerm时，不要重置time-out timer，保证及时time out以转变为Candidate。
...
**Candidate**是一个中间角色，当Follower time out后转变为Candidate角色，参与选举。
...
`Figure2`从`Request`参数、`Response`参数、接收者的实现上都做了详细的说明。

2A的测试实现时先不用考虑日志最新的问题。
...
复杂主要体现在两点：
1. log inconsistencies的判断，follower的日志是可能比leader更多的，此时需要inconsistencies的判断是否响应该leader的日志。  
2. 更新自身的commitIndex

注：
论文里提到了一些log inconsistencies后的优化方法，以保证尽快与leader的log一致，但是认为这个优化不一定需要。在lab2里需要实现这点。因此我们的AppendEntries RPC的Result跟Figure2相比，增加了两个参数
...
作为分布式系统的入门菜鸟一枚 + go的初学者，应该还有一些对论文理解不充分的地方。在实现过程中采用了很多trick的方案，经常压抑不住内心的洪荒之力想要重构之前的代码。不过由于时间原因，还是只好基于最初的版本，一点点的修改了出来。当然还是踩了一些坑的，这里分享一下，也欢迎留言讨论。

+ 日志要清楚：因为进程可能在三种角色间转换，日志量会很大，可以参考下是否有更好的trace方式，我在每条日志里都会打印*rf.me*，通过`grep me:1`可以看到重点分析一个实例的日志。
...
但是还是一不小心踩坑了，原因是即使是submessage，同样需要首字母大写。例如`AppendEntriesArgs`这么定义：
