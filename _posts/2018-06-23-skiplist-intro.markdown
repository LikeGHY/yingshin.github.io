...
很明显，多个列表可以优化查找的时间复杂度，那么需要多少个列表才能达到O(lgn)的时间复杂度。
...
理想skiplist是每隔两个元素，则该元素会在下一个链表出现。上面提了实际肯定没法这么操作，而是采用了概率论的做法，有时候数学家的做法就这么简单可依赖：我们用抛硬币来解决吧😱。
