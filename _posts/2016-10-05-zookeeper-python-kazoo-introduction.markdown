...
在[zookeeper的c客户端](http://izualzhy/2016/10/02/zookeeper-c-api-introduction)里，更多的是提供了食材，至于最后如何完成食谱(recipe)，则由使用方自己根据场景实现。因此经常给人一种接口很多，貌似实现了分布式的协调服务，但是却又需要自己实现一遍的感觉。
在[zookeeper的c客户端](http://izualzhy/2016/10/02/zookeeper-c-api-introduction)里，更多的是提供了食材，至于最后如何完成食谱(recipe)，则由使用方自己根据场景实现。因此经常给人一种接口很多，貌似实现了分布式的协调服务，但是却又需要自己实现一遍的感觉。
...
以上set watch参数跟c客户端相同，都是单次触发。其实并不难理解，因为何时通知是在服务端确定的。但是借助python[装饰器](http://izualzhy/2015/08/04/python-decorator-notes)，Kazoo在客户端实现了触发后自动继续注册监视点的方式。
以上set watch参数跟c客户端相同，都是单次触发。其实并不难理解，因为何时通知是在服务端确定的。但是借助python[装饰器](http://izualzhy/2015/08/04/python-decorator-notes)，Kazoo在客户端实现了触发后自动继续注册监视点的方式。
