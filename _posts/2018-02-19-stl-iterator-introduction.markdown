...
stl-iterator实际上是一个很庞大的话题，细细展开已经不是一篇文章两万字可以讲清楚的了，例如iterator的设计思想，tag dispatcher，traits的设计，包括half-open的range:[begin, end)已经成为我们对于range一个标准的理解。而stl还在不断的完善这个featuer，例如`Move Iterators`的设计。伴随stl-algorithm function，iterator可以呈现更大的空间，之后会写一篇文章总结下stl里的algorithm。

当然iterator的使用也有很多需要注意的地方：`singular past-the-end out-of-range dangling iconsistent`等，如果有时间也想总结一篇。
