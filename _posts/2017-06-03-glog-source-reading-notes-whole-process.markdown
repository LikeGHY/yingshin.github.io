...
这篇笔记接着[上篇笔记:glog源码解析一：整体结构](http://izualzhy.cn/c/cpp/2017/06/03/glog-source-reading-notes)，继续介绍`LOG(INFO)`调用后的完整过程，通过梳理这个过程，能覆盖到大部分的代码。当然为了便于理解，一些细节和分支都做了简化。
...
这里重点关注`LogMessageData`，这是一个负责存储日志数据以及组织写入的类。
...
`LogStream stream_`负责流式数据的写入，继承自`std::ostream`，实现了自己的`streambuf`，使用的数据缓冲区是`message_text_`，用于接收流式的日志数据并存储。可以看到`message_text_`是定长的字符串。关于这个长度注释也比较有意思（大意就是这个魔数你就别猜原因了，就是个武断的值）：
...
日志数据的后续接收通过`LogStream stream_;`完成，由于继承自`std::ostream`因此天然支持`<<`的流式日志输入，因此"hello world"字符串最后存储到了`message_text_`。
...
```
  ...
  if (FLAGS_logtostderr || !IsGoogleLoggingInitialized()) {
    ...
    LogDestination::MaybeLogToEmail(data_->severity_, data_->message_text_,
                                    data_->num_chars_to_log_);
    //写到用户自定义的sink输出，去掉了默认添加的logprefix，即severity，ts，线程id，文件basename等
    LogDestination::LogToSinks(data_->severity_,
                               data_->fullname_, data_->basename_,
                               data_->line_, &data_->tm_time_,
                               data_->message_text_ + data_->num_prefix_chars_,
...
`sinks_`是`LogDestination`的静态成员变量，用于实现用户自定义的输出方式。
...
标准日志的输出到这里就结束了。对于用户自定义的输出方式，则是通过`LogDestination::LogToSinks`完成。
