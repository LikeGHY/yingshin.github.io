...
Protbuf序列化之后的数据如此紧凑，得益于采用的encoding方法。首先介绍下用于小整数的Varint编码方案。
...
1. 对数字1进行varint编码后的结果为`0000 0001`，占用1个字节。相比`int32`字节前面补很多0，节省了3个字节的空间。这里也可以看出Varint的想法就是 **以标志位替换掉高字节的若干个0**
...
第一个字节为`10101100`，第二个字节为`00000010`（不够7位高位补0）。

具体的变化步骤：

![number_300_varint.png](/assets/images/number_300_varint.png)

文章开始例子使用的数值150，varint编码后的结果为`10010110 00000001`，即`0x96 0x01`。

根据上面的规则，对应的代码如下（`coded_stream.cc`）：
...
可以看到对于较大的数字(1<<28)，使用的字节数由4个变成了5个，同时对于负数也有影响。我们先来解释下本文最开始编码字符`08 96 01`如何产生的问题。
...
可以看到varint编码也应用在了key的计算上，使用非常频繁，或许是基于这个原因，pb里实现了一种性能更高的方案（`coded_stream.cc`）。
...
在普遍的使用场景下，整型只需要`int32/int64/uint32/uint64`就可以了，一般不需要考虑编码后的大小。本文主要抱着知其然知其所以然的态度了解下pb封包的过程，这种紧凑的编码方式也有助于我们设计自己的序列化方案。同时对于负数、大整数较多的业务场景要注意对应proto上的设计。
